...
const AddressModal = ({ setShowAddressModal, editAddress = null }) => {
        ...
        try {
            if (editAddress) {
                ...
                const res = await fetch('/api/address/update', {
                    ...
                    body: JSON.stringify({ addressId: editAddress.id, ...address })
                })
    ...
    return (
        <form onSubmit={e => toast.promise(handleSubmit(e), { loading: editAddress ? 'Updating...' : 'Adding Address...' })} className="fixed inset-0 z-50 bg-white/60 backdrop-blur h-screen flex items-center justify-center">
            <div className="flex flex-col gap-5 text-slate-700 w-full max-w-sm mx-6">
                <h2 className="text-3xl">{editAddress ? 'Edit' : 'Add New'} <span className="font-semibold">Address</span></h2>
